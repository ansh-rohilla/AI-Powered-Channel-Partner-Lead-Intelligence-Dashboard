...
def seed_database():
    ...
    with app.app_context():
        ...
        # Load CSVs
        partners_csv_path = os.path.join("data", "sample", "partners.csv")
        leads_csv_path = os.path.join("data", "sample", "leads.csv")

        if not os.path.exists(partners_csv_path) or not os.path.exists(leads_csv_path):
            print("Error: Mock CSV data files not found. Run generate_sample_data.py first.")
            return
